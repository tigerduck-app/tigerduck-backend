    ...
    * devices with a non-null `device_token_hex` (no standard token → no
    ...
    stmt = (
        ...
        .where(
            ...
            # Phase 4c: devices linked to a logged-in v3 user are served
            # by the user-level push_jobs flow — skip them here so the
            # dual-track migration doesn't double-push bulletins.
